...
function testConfig(input) {
	...
	testFn(input.description, (t) => {
		const config = {
			...
			rules: {
				'plugin/property-groups-structure': input.config
			}
		};
...
testConfig({
	...
	message: 'Invalid option "[{"emptyLineBefore":true,"order":"flexible","properties":["border-bottom","font-style"]}]" for rule plugin/property-groups-structure',
});
...
testConfig({
	...
	message: 'Invalid option "[{"emptyLineBefore":"always","order":"flexible","properties":null}]" for rule plugin/property-groups-structure',
});
